...
//	DWORD dll_and_eip;
	...
	for (unsigned int i = 0; i < updated_thread_list.number_of_members; ++i) {
		open_thread = OpenThread(THREAD_QUERY_INFORMATION | THREAD_GET_CONTEXT | THREAD_SET_CONTEXT | THREAD_SUSPEND_RESUME, false, *(updated_thread_list.tids + i));
		if (-1 == SuspendThread(open_thread)) {
			fprintf(stderr, "unable to suspend the thread");
			getchar();
			return FAIL;
		}
		thread_context.ContextFlags = CONTEXT_FULL;
		if (!GetThreadContext(open_thread, &thread_context)) {
			fprintf(stderr, "Cant get thread context,ERROR:%d",GetLastError());
			getchar();
			return FAIL;
		}
		
		original_thread_context = thread_context.Eip;
		thread_context.Eip = (DWORD)GetProcAddress(GetModuleHandle(TEXT("kernel32.dll")), "LoadLibraryW");

		if (!(WriteProcessMemory(specific_process,(LPVOID)(thread_context.Esp - sizeof(alloc_space)), &alloc_space, sizeof(alloc_space), NULL))) {
			printf("Cant write to memory of the remote process, Exsiting ! ErrCode:%d", GetLastError());
			CloseHandle(specific_process);
			getchar();
			return FAIL;
		...
		if (!(WriteProcessMemory(specific_process, (LPVOID)(thread_context.Esp - sizeof(alloc_space) - sizeof(original_thread_context)), &original_thread_context, sizeof(original_thread_context), NULL))) {
			printf("Cant write to memory of the remote process, Exsiting ! ErrCode:%d", GetLastError());
			CloseHandle(specific_process);
			getchar();
			return FAIL;
		...
		thread_context.Esp -= sizeof(alloc_space) - sizeof(original_thread_context);

		if (!SetThreadContext(open_thread, &thread_context)) {
			fprintf(stderr, "cant set thread context");
			getchar();
			return FAIL;
		}

		if (-1 == ResumeThread(open_thread)) {
			fprintf(stderr, "Cant resume Thread");
			getchar();
			return FAIL;
		}
	}
